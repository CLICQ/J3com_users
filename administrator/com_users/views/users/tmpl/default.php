	...
	<table class="table table-striped">
		<thead>
			<tr>
				...
				<th class="left">
					<?php echo JHtml::_('searchtools.sort', 'ФИО', 'a.name', $listDirn, $listOrder); ?>
				</th>
		...
		?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td class="center">
				<img src="<?php if(''!=$item->avatar){ echo (string) $item->avatar;}else {echo $item->avatar.'/avatar/noavatar.jpg';} ?>" alt="Аватар пользователя" width="60px" height="60px"> 
				</td>
				<td>
					<?php if ($canEdit) : ?>
						<a href="<?php echo JRoute::_('index.php?option=com_users&task=user.edit&id='.(int) $item->id); ?>" title="<?php echo JText::sprintf('COM_USERS_EDIT_USER', $this->escape($item->name)); ?>">
							<?php echo $this->escape($item->name.' '.$item->lastname); ?></a>
					<?php else : ?>
				...
				<td class="center">
					...
					if('1'==$item->type2)
					echo "<p>Эксперт</p>" ;
					if('0'==$item->type1 AND '0'==$item->type2)
					echo "<p>Наблюдатель</p>";
					if(''==$item->type1 AND ''==$item->type2)
					echo "<p>Наблюдатель</p>";
